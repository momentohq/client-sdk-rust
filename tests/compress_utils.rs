...
mod test {
    ...
    use momento::compression_utils::{compress_json_str, decompress_json_bytes};
    ...
    fn test_compress_json() {
        ...
        let compressed = compress_json_str(&json_str).expect("compression failed");
        ...
        let decompressed =
            decompress_json_bytes(compressed.as_slice()).expect("decompression failed");
        assert_eq!(json_str.as_bytes(), decompressed);
        assert_eq!(json_str, String::from_utf8(decompressed).expect("failed to convert from bytes to utf8"))
    }
}
