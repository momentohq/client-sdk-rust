...
#[tokio::test]
async fn flush_nonexistent_cache_returns_not_found() {
    let client = CACHE_TEST_STATE.client.clone();
    let cache_name = "fake-cache-".to_string() + &Uuid::new_v4().to_string();
...
#[tokio::test]
async fn flush_existing_cache_returns_success() {
    let client = CACHE_TEST_STATE.client.clone();
    let cache_name = CACHE_TEST_STATE.cache_name.clone();
    ...
    let get_result1 = client.get(cache_name.to_string(), "key1").await.unwrap();
    assert_eq!(
        get_result1,
        Get::Hit {
    ...
    let get_result2 = client.get(cache_name.to_string(), "key2").await.unwrap();
    assert_eq!(
        get_result2,
        Get::Hit {
    ...
    // Flush the cache
    let result = client.flush_cache(cache_name.to_string()).await.unwrap();
    assert_eq!(result, FlushCache {});

    // Verify that the elements were flushed from the cache
    let get_result3 = client.get(cache_name.to_string(), "key1").await.unwrap();
    assert_eq!(get_result3, Get::Miss {});
    let get_result4 = client.get(cache_name.to_string(), "key2").await.unwrap();
