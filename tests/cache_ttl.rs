...
use momento::cache::{DecreaseTtl, IncreaseTtl, ItemGetTtl, SetRequest, SortedSetPutElementsRequest, UpdateTtl};
use momento::{CollectionTtl, MomentoErrorCode, MomentoResult};
use momento_test_util::{unique_string, CACHE_TEST_STATE};

#[tokio::test]
async fn item_get_ttl_invalid_cache_name() -> MomentoResult<()> {
...
#[tokio::test]
async fn item_get_ttl_nonexistent_cache() -> MomentoResult<()> {
    let client = &CACHE_TEST_STATE.client;
...
#[tokio::test]
async fn item_get_ttl_nonexistent_key() -> MomentoResult<()> {
    let client = &CACHE_TEST_STATE.client;
...
#[tokio::test]
async fn item_get_ttl_for_a_scalar() -> MomentoResult<()> {
    let client = &CACHE_TEST_STATE.client;
    let cache_name = CACHE_TEST_STATE.cache_name.as_str();
    let key_uuid = unique_string("key");
    let key = key_uuid.as_str();
...
#[tokio::test]
async fn item_get_ttl_for_a_sorted_set() -> MomentoResult<()> {
    let client = &CACHE_TEST_STATE.client;
    let cache_name = CACHE_TEST_STATE.cache_name.as_str();
    let key_uuid = unique_string("key");
    let key = key_uuid.as_str();
    ...
    client
        .send_request(
            SortedSetPutElementsRequest::new(cache_name, key, vec![("abc", 12.3), ("xyz", 32.1)])
...
#[tokio::test]
async fn increase_ttl_invalid_cache_name() -> MomentoResult<()> {
    let client = &CACHE_TEST_STATE.client;
    let result = client
...
#[tokio::test]
async fn increase_ttl_nonexistent_cache() -> MomentoResult<()> {
    let client = &CACHE_TEST_STATE.client;
    let cache_name = unique_string("fake-cache");
    let result = client
...
#[tokio::test]
async fn increase_ttl_nonexistent_key() -> MomentoResult<()> {
    let client = &CACHE_TEST_STATE.client;
    let cache_name = CACHE_TEST_STATE.cache_name.as_str();
    let key_uuid = unique_string("key");
    let key = key_uuid.as_str();
    let result = client.increase_ttl(cache_name, key, Duration::from_secs(10)).await?;
    assert_eq!(result, IncreaseTtl::Miss {});
    Ok(())
}

#[tokio::test]
async fn increase_ttl_only_increases_ttl_for_existing_key() -> MomentoResult<()> {
    let client = &CACHE_TEST_STATE.client;
    let cache_name = CACHE_TEST_STATE.cache_name.as_str();
    let key_uuid = unique_string("key");
    let key = key_uuid.as_str();
    ...
    // Set a higher TTL
    let result = client.increase_ttl(cache_name, key, Duration::from_secs(20)).await?;
...
#[tokio::test]
async fn decrease_ttl_invalid_cache_name() -> MomentoResult<()> {
    let client = &CACHE_TEST_STATE.client;
    let result = client
...
#[tokio::test]
async fn decrease_ttl_nonexistent_cache() -> MomentoResult<()> {
    let client = &CACHE_TEST_STATE.client;
    let cache_name = unique_string("fake-cache");
    let result = client
...
#[tokio::test]
async fn decrease_ttl_nonexistent_key() -> MomentoResult<()> {
    let client = &CACHE_TEST_STATE.client;
    let cache_name = CACHE_TEST_STATE.cache_name.as_str();
    let key_uuid = unique_string("key");
    let key = key_uuid.as_str();
    let result = client.decrease_ttl(cache_name, key, Duration::from_secs(1)).await?;
    assert_eq!(result, DecreaseTtl::Miss {});
    Ok(())
}

#[tokio::test]
async fn decrease_ttl_only_decreases_ttl_for_existing_key() -> MomentoResult<()> {
    let client = &CACHE_TEST_STATE.client;
    let cache_name = CACHE_TEST_STATE.cache_name.as_str();
    let key_uuid = unique_string("key");
    let key = key_uuid.as_str();
    ...
    // Set a lower TTL
    let result = client.decrease_ttl(cache_name, key, Duration::from_secs(5)).await?;
    ...
    let result = client.decrease_ttl(cache_name, key, Duration::from_secs(10)).await?;
...
#[tokio::test]
async fn update_ttl_invalid_cache_name() -> MomentoResult<()> {
    let client = &CACHE_TEST_STATE.client;
    let result = client
...
#[tokio::test]
async fn update_ttl_nonexistent_cache() -> MomentoResult<()> {
    let client = &CACHE_TEST_STATE.client;
    let cache_name = unique_string("fake-cache");
    let result = client
...
#[tokio::test]
async fn update_ttl_nonexistent_key() -> MomentoResult<()> {
    let client = &CACHE_TEST_STATE.client;
    let cache_name = CACHE_TEST_STATE.cache_name.as_str();
    let key_uuid = unique_string("key");
    let key = key_uuid.as_str();
    let result = client.update_ttl(cache_name, key, Duration::from_secs(5)).await?;
    assert_eq!(result, UpdateTtl::Miss {});
    Ok(())
}

#[tokio::test]
async fn update_ttl_overwrites_ttl_for_existing_key() -> MomentoResult<()> {
    let client = &CACHE_TEST_STATE.client;
    let cache_name = CACHE_TEST_STATE.cache_name.as_str();
    let key_uuid = unique_string("key");
    let key = key_uuid.as_str();
    ...
    client.update_ttl(cache_name, key, Duration::from_secs(20)).await?;
